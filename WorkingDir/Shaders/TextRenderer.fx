...
void GS(point DummyIn dummyInput[1], inout TriangleStream<PSSceneIn> triStream)
{
	...
	for(int i = 0; i < NrOfChars; i++)
	{
		...
		//bottom left
		output.Position = basepos + float4(width, -height, 0, 0);
		output.TexCoord = float2(texx, 1);
		triStream.Append(output);
		
		//bottom right
		output.Position = basepos + float4(width + thiswidth, -height, 0, 0);
		output.TexCoord = float2(texx + texxwidth, 1);
		triStream.Append(output);

		//top left
		output.Position = basepos + float4(width, 0, 0, 0);
		output.TexCoord = float2(texx, 0);
		triStream.Append(output);

		//top right
		output.Position = basepos + float4(width + thiswidth, 0, 0, 0);
		output.TexCoord = float2(texx + texxwidth, 0);
		triStream.Append(output);
