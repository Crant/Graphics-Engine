...
void DebugMtxInfo(DWORD res)
{
	if(res != WAIT_OBJECT_0)
	{
		...
		if(res == WAIT_ABANDONED)
			CJen::Debug("MUTEX FAILED: ERROR: WAIT_ABANDONED!");
		else if(res == WAIT_TIMEOUT)
			CJen::Debug("MUTEX FAILED: ERROR: WAIT_TIMEOUT!");
		else if(res == WAIT_FAILED)
			CJen::Debug("MUTEX FAILED: ERROR: WAIT_FAILED!");
...
Process::~Process()
{
	...
	*/
	...
	if(this->debug)
		CJen::Debug("ERROR: Proc: " + CJen::convertNrToString((float)this->id) + " deleted itself while another process was doing work with it.");
	this->debug = true;
...
ProcessEvent* Process::WaitEvent()
{
	if(this->ProcMtx)
	{
		...
		CJen::Debug("Mutex is broken / hasn't been created / has been closed for proc: " + CJen::convertNrToString((float)this->id));
	
	if(this->debug)
		CJen::Debug("ERROR: Proc: " + CJen::convertNrToString((float)this->id) + " Mutex for WaitEvent failed, multiple procs modifying data.");
	this->debug = true;
	...
	if(sleep)
	{
		...
		if(this->debug)
			CJen::Debug("ERROR: Proc: " + CJen::convertNrToString((float)this->id) + " Mutex for WaitEvent, second,  failed, multiple procs modifying data.");
		this->debug = true;
	}
...
ProcessEvent* Process::PeekEvent()
{
	...
	if(this->debug)
		CJen::Debug("ERROR: Proc: " + CJen::convertNrToString((float)this->id) + " Mutex for PeekEvent failed, multiple procs modifying data.");
	this->debug = true;
...
void Process::PutEvent(ProcessEvent* ev, bool important)
{
	...
	if(go)
	{
		...
		if(this->debug)
			CJen::Debug("ERROR: Proc: " + CJen::convertNrToString((float)this->id) + " Mutex for PutEvent failed, multiple procs modifying data.");
		this->debug = true;
		...
		{
			CJen::Debug("Warning, EventQueue of process " + CJen::convertNrToString((float)this->id) + " has " + 
				CJen::convertNrToString((float)this->EvQueue->size()) + " unread events.");
